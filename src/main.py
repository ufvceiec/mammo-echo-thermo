...
join_models = [Join(high_model[index], medium_model[index], low_model[index], path=f"./output/state_{random_state}/join/fold_{index}") for index in range(len(high_model))]

[join_models[index].get_weighted_average(test_generator, iterations=100, tolerance=1e-7) for index in range(len(high_model))]

[join_models[index].evaluate(test_generator, title="test_generator") for index in range(len(high_model))]
